...
几何 **`dx = 0.10 m`、`dy = 0`**（当时的占位值）；小 yaw 行程 `[−25°, +20°]`；
`θ*（离心平衡点）= −29.9°`（在行程之外）。

> ⚠ **几何已更新（2026-09 后期）**: 实测确认两轴是 **`(dx, dy) = (0, 0.07)` m**
> （横向无偏置、小 yaw 轴在大 yaw 轴**前方** 0.07 m），仓库默认值已随之改为 `(0, 0.07)`。
> 本目录里的这批 CSV 是**用旧占位几何 `(0.10, 0)` 生成的**，因此:
> - 复现本目录的结论时**必须显式**给 `--dx=0.1 --dy=0`（否则 `P` 会因几何不匹配而系统性偏移）；
> - 这些结论**仍然有效**（它们陈述的是"在 d=(0.1,0) 这套几何下、这批数据能辨识到什么"），
>   但**不能**当作新几何下的验证结果。新几何的验证见 `data/sysid/`（用当前默认值生成的数据）。
