...
3. **模式位语义**：**`1 = 仅力矩`**、**`2 = 力矩 + 位置/速度内环`**；
   **其余取值（含 `0`）= 非法模式 ⇒ 该关节力矩按 0 处理**（电控不猜测上位机意图，
   并把目标角/目标角速度一并清零）。与旧版单轴示例的 `yaw_torque_only_mode`（1 = 仅力矩）一致。
   ⚠ 大 yaw 指令会被 MCU1 **原样转发给 MCU2**，两片固件必须同版本（旧固件里 `0` = 内环模式）。
