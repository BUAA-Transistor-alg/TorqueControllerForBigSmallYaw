...
typedef struct TcbsDualYawJointLimits_C {
    ...
    double min_angle;        // rad，机械行程下界（小 yaw 默认 −35°；**允许非对称**）
    double max_angle;        // rad，机械行程上界（小 yaw 默认 +35°）
...
typedef struct TcbsDualYawMpcConfig_C {
    ...
    //       （当前行程对称 ±35° ⇒ 0），与 C++ defaultMpcConfig() 的默认值一致；
...
typedef struct TcbsEstimatorConfig_C {
    ...
    // ★ 本结构体**新增了 4 个字段**（原 `rate_lpf_alpha` 拆成两个 + 大 yaw 编码器支路三项）
    //   ⇒ sizeof 变大；调用方若用旧头文件编译，`tcbs_robot_comm_check_abi()` 会报
    //   "结构体布局不匹配 TcbsEstimatorConfig_C"（**有意的显式失败**，不是静默错位）
...
// yaw_*_mode: **1 = 仅力矩**，**2 = 力矩 + 电控位置/速度内环**，
//             其余取值（含 0）= 非法模式 ⇒ 电控把该关节力矩按 0 处理（见 mcu::YawMode）
int tcbs_robot_comm_send_to_mcu(TcbsRobotCommHandle* handle,
                           ...
                           float   yaw_small_target_angle,   // 小 yaw 关节目标角，rad（行程 ±35°，由电控侧再限位）
...
// big_joint_angle: 大 yaw 关节角（相对底盘，多圈）；small_joint_angle: 小 yaw 关节角（行程 ±35°）
