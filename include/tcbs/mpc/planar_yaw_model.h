...
struct ModelParams {
    ...
    //    那里是权威来源、并记录了可信度分级）──
    double Jbig_eff = 0.051663;      // 大 yaw 侧惯量（含 m_u|d|²）      kg·m²
    double Js       = 0.004578;      // 上装绕小 yaw 轴总惯量            kg·m²
    double Px       = 0.000288;      // 上装一阶矩 m_u·ρ_x               kg·m（★ 本批数据不可辨识）
    double Py       = 0.021925;     // 上装一阶矩 m_u·ρ_y               kg·m（★ 同上）
    double fcBig    = 0.261903, fvBig   = 0.116187;   // 大 yaw 库仑/粘滞摩擦（fv 可疑）
    double fcSmall  = 0.021027, fvSmall = 0.038137;   // 小 yaw 库仑/粘滞摩擦（fv 可疑）
    ...
    double Pbx = -0.013844, Pby = 0.133088;
    ...
    double backlash_delta    = 0.018163; // 背隙总宽度 δ (rad)，≈5.53°（实车辨识）
    double backlash_k        = 23.450883; // 接触刚度 (N·m/rad)（"弹性范围很小" ⇒ 较大）
    double backlash_c        = 7.702286; // 接触阻尼 (N·m·s/rad)；与 k 共线、可信度低（见 params.h）
    ...
    //   想严格物理就把 γ 置 0。
    double Jmotor            = 0.025028; // 电机侧惯量（**折算到关节侧**），kg·m²
    double fcMotor           = 0.059399; // 电机侧库仑摩擦（背隙内电机几乎空载 ⇒ 单独一组；可信度低）
    double fvMotor           = 0.066934; // 电机侧粘滞摩擦（同上：与 fcMotor 互换得很厉害）
