...
//
// 波特率: 可用编译期宏 SERIAL_BAUD_RATE 覆盖（默认 B115200）。
// 注意: 本工程双向帧约 58B(MCU→PC) + 41B(PC→MCU)，100Hz 时双向合计 ≈9.9 kB/s，
// 在 115200(≈11.5 kB/s) 下已接近饱和 → 建议电控与上位机同时改用 460800，
// 只需在本文件与电控侧各改一处。
//
#ifndef TCBS_SERIALPROTOCOL_HPP
#define TCBS_SERIALPROTOCOL_HPP
...
SerialProtocol<S, R, C, P, L>::SerialProtocol(std::function<void(const R&)> callback, bool auto_start)
    : fd_(-1), serialDataCallback(callback)   // 初始化顺序必须与成员声明顺序一致（否则 -Wreorder）
{
...
void SerialProtocol<S, R, C, P, L>::initializeSerial() {
    ...
    cfsetospeed(&tty, SERIAL_BAUD_RATE); cfsetispeed(&tty, SERIAL_BAUD_RATE);
